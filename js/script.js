...
const prixTotal = document.getElementById("prix-total");
const livraisonTypeRadios = document.querySelectorAll(
  'input[name="livraison-type"]'
);
...
function updateTotals() {
  ...
  const selectedLivraisonType = document.querySelector(
    'input[name="livraison-type"]:checked'
  );

  if (selectedLivraisonType) {
    livraisonTypeText = selectedLivraisonType.value;
    livraisonUnitPrice =
      livraisonTypeText === "relais"
        ? prixLivraisonRelais
        : prixLivraisonDomicile;
  }
...
//Gérer l'ajout ou la suppression d'une ligne
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function addRow() {
    const lastProductRow = document.querySelector(
      "#checkout tbody tr:last-child:not([colspan])"
    );
    ...
    // Mettre à jour les totaux après l'ajout d'une ligne
    updateTotals();
  }
  ...
  function removeRow(button) {
    ...
    // Mettre à jour les totaux après la suppression d'une ligne
    updateTotals();
  }
